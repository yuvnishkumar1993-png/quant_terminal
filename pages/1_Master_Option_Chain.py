    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for s_str, obj in oc_map.items():
                ...
                ce_bid = ce.get("bid_price", ce_ltp * 0.99)
                ce_ask = ce.get("ask_price", ce_ltp * 1.01)
                pe_bid = pe.get("bid_price", pe_ltp * 0.99)
                pe_ask = pe.get("ask_price", pe_ltp * 1.01)
                
                ce_spread = round(abs(ce_ask - ce_bid), 2)
                pe_spread = round(abs(pe_ask - pe_bid), 2)
                ce_spread_pct = round((ce_spread / ce_ltp) * 100, 2) if ce_ltp > 0 else 0.0
                pe_spread_pct = round((pe_spread / pe_ltp) * 100, 2) if pe_ltp > 0 else 0.0
                
                records.append({
                    "CE Spread %": ce_spread_pct,
                    "CE LTP": ce_ltp,
                    "CE %Chg": float(ce.get("net_change", 0.0)),
                    ...
                    "PE %Chg": float(pe.get("net_change", 0.0)),
                    "PE LTP": pe_ltp,
                    "PE IV": pe_iv if pe_iv > 0.5 else fallback_iv,
                    "PE Spread %": pe_spread_pct,
                    "Raw_CE_OI": ce_oi,
                    "Raw_PE_OI": pe_oi
                })
...
if chain_df.empty:
    ...
    for s in strikes_arr:
        ...
        skew_boost = (distance_from_spot / live_spot) * 35.0 if s < live_spot else (distance_from_spot / live_spot) * 20.0
        c_iv_val = round(def_iv + max(0.5, skew_boost * 0.4), 2)
        p_iv_val = round(def_iv + max(1.0, skew_boost * 0.8), 2)
        
        c_ltp = max(5.0, round(float(np.random.normal(100, 50)), 2))
        p_ltp = max(5.0, round(float(np.random.normal(100, 50)), 2))
        
        mock_recs.append({
            "CE Spread %": round(np.random.uniform(0.1, 1.5), 2),
            "CE LTP": c_ltp, "CE %Chg": round(np.random.uniform(-10, 15), 2), "CE IV": c_iv_val, 
            "CE Vol": np.random.randint(50000, 800000), "CE Chg OI": np.random.randint(-15000, 20000), "CE OI (L)": round(c_oi/100000, 2),
            "STRIKE": int(s), 
            "PE OI (L)": round(p_oi/100000, 2), "PE Chg OI": np.random.randint(-15000, 20000), "PE Vol": np.random.randint(50000, 800000),
            "PE %Chg": round(np.random.uniform(-10, 15), 2), "PE LTP": p_ltp, "PE IV": p_iv_val, "PE Spread %": round(np.random.uniform(0.1, 1.5), 2),
